...
class FingerprintWords:
    """
    SHA-256 指纹转换为易读的英文单词列表（类似 Signal 安全码）
    使用 BIP39 标准英文词表（2048 个单词）
    """
    
    # 从 english.txt 加载 BIP39 词表
    _wordlist = None
        ...
        if cls._wordlist is None:
            try:
                ...
                if len(cls._wordlist) != 2048:
                    raise ValueError(f"内嵌词表应包含 2048 个单词，但获得 {len(cls._wordlist)} 个")
            except Exception as e:
                try:
                    ...
                    if len(cls._wordlist) != 2048:
                        raise ValueError(f"词表应包含 2048 个单词，但获得 {len(cls._wordlist)} 个")
                except Exception as fallback_error:
                    raise RuntimeError(f"无法加载 BIP39 词表: {e}; fallback: {fallback_error}")
        return cls._wordlist
    
    @staticmethod
    def fingerprint_to_words(fingerprint_hex: str, word_count: int = 6) -> list:
        """
        将 SHA-256 十六进制指纹转换为单词列表
        
        Args:
            fingerprint_hex: 64 位十六进制字符串（32 字节 SHA-256 哈希）
            word_count: 要生成的单词数（默认 6，对应 66 bit）
        ...
        if not isinstance(fingerprint_hex, str) or len(fingerprint_hex) != 64:
            raise ValueError(f"指纹必须是 64 位十六进制字符串，获得: {fingerprint_hex}")
        
        if word_count < 1 or word_count > 24:
            raise ValueError(f"单词数必须在 1-24 之间，获得: {word_count}")
        
        try:
            fingerprint_bytes = bytes.fromhex(fingerprint_hex)
        except ValueError as e:
            raise ValueError(f"无效的十六进制指纹: {e}")
        
        wordlist = FingerprintWords._load_wordlist()
        
        # 计算需要的比特数：word_count * 11
        ...
        for i in range(word_count):
            ...
            if index >= len(wordlist):
                raise RuntimeError(f"索引越界: {index} >= {len(wordlist)}")
