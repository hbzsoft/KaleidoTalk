...
def send_msg(sock, obj):
    """Send a JSON message in format: [4-byte length][JSON bytes]."""
    state = _get_socket_state(sock)
    with _STATE_LOCK:
        state['send_seq'] += 1
        seq = state['send_seq']
...
def recv_msg(sock):
    """Receive one JSON message; return dict or None when closed."""
    ...
    try:
        ...
        raise ValueError('invalid message sequence or timestamp') from e
    ...
    with _STATE_LOCK:
        ...
        if abs(now_ms - timestamp) > _MAX_CLOCK_SKEW_MS:
            raise ValueError('message timestamp out of allowed range')
